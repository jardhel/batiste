 ...
 * successful index, significantly reducing re-indexing time for large
 * codebases. Falls back to file-hash/mtime comparison when git is not
 * available.
 */

import { readFile, writeFile, mkdir } from 'fs/promises';
import { join, dirname } from 'path';
import { GitTracker, type GitDiffResult } from './git-tracker.js';
import { FileIndex, type IndexedFileInfo } from './file-index.js';
import { Crawler, type FileInfo } from './crawler.js';
...
export class GitAwareIndexer {
  ...
  async planIncrementalIndex(
    ...
    if (!this.gitTracker.isGitRepository()) {
      this.log.info(
        'Not a git repository, using file-based change detection'
      );
    ...
    if (!this.indexState?.lastIndexedCommit || !headCommit) {
      this.log.info(
        'No previous index state or HEAD commit, doing full index'
      );
      return this.createFullIndexPlan(
        currentFiles,
        'No previous index state'
      );
    }
    ...
    if (this.indexState.branch !== currentBranch) {
      this.log.info(
        `Branch changed from ${this.indexState.branch} to ${currentBranch}`
      );
    }
    ...
    try {
      gitDiff = await this.gitTracker.getDiff(
        this.indexState.lastIndexedCommit,
        headCommit
      );
    } catch (err) {
      this.log.warn(
        'Failed to get git diff, falling back to file-based detection:',
        err
      );
      return this.planWithoutGit(currentFiles);
    }
    ...
    if (includeUncommitted) {
      const workingTreeChanges =
        await this.gitTracker.getWorkingTreeChanges();
  ...
  private planWithoutGit(
    ...
    return {
      ...
      isFullReindex:
        !this.fileIndex.isLoaded() || this.fileIndex.size === 0,
      reason: `File-based incremental: ${changedFiles.length} changed, ${deletedFiles.length} deleted`,
      stats: {
        totalFilesInRepo: currentFiles.length,
        filesToAdd: changedFiles.filter(
          f => !this.fileIndex.getFile(f.relativePath)
        ).length,
        filesToUpdate: changedFiles.filter(
          f => this.fileIndex.getFile(f.relativePath) !== undefined
        ).length,
   ...
   */
  private mergeGitDiffs(
    target: GitDiffResult,
    source: GitDiffResult
  ): void {
    ...
    for (const modified of source.modified) {
      if (
        !target.modified.includes(modified) &&
        !target.added.includes(modified)
      ) {
  ...
  updateFileIndex(
    relativePath: string,
    info: Omit<IndexedFileInfo, 'relativePath'>
  ): void {
