/**
 * File system crawler with git-aware prioritization.
 *
 * Scans the project directory for source files, respects .gitignore and
 * .claudeignore patterns, and assigns git priority scores so that
 * recently modified and uncommitted files are processed first.
 */
...
import { Config, getLanguageFromExtension, LANGUAGE_EXTENSIONS } from '../utils/config.js';
...
export interface FileInfo {
  ...
  size: number;
  modifiedAt: Date;
  /** Higher = more recently modified in git */
  gitPriority?: number;
...
export class Crawler {
  ...
  async crawl(): Promise<CrawlResult> {
    ...
    this.log.info(
      `Found ${files.length} files across ${Object.keys(languages).length} languages`
    );
    if (gitAware) {
